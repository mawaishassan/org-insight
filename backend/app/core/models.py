...
class CustomReportSection(Base):
    """KPI section within a Custom Report."""

    __tablename__ = "custom_report_sections"
    ...
    custom_report = relationship("CustomReport", back_populates="sections")
    kpi = relationship("KPI")
    fields = relationship(
        "CustomReportField",
...
class CustomReportField(Base):
    """Specific field placed in a section (supports cross-KPI field movement)."""

    __tablename__ = "custom_report_fields"

    id = Column(Integer, primary_key=True, index=True)
    custom_report_id = Column(
        Integer, ForeignKey("custom_reports.id", ondelete="CASCADE"), nullable=False, index=True
    )
    custom_report_section_id = Column(
        Integer, ForeignKey("custom_report_sections.id", ondelete="CASCADE"), nullable=False, index=True
    )
    kpi_field_id = Column(
        Integer, ForeignKey("kpi_fields.id", ondelete="CASCADE"), nullable=False, index=True
    )
...
class CustomReportAssignment(Base):
    """Permission assignment for custom report templates."""

    __tablename__ = "custom_report_assignments"
    ...
    __table_args__ = (
        UniqueConstraint("custom_report_id", "user_id", name="uq_custom_report_user"),
    )
